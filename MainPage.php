...
class MainPage
{
    ...
    private function setup()
    {
        ...
        $this->strava = new JoanMcGalliard\EddingtonAndMore\Strava($stravaClientId, $stravaClientSecret, $this->echoCallback);
        $this->myCyclingLog = new JoanMcGalliard\EddingtonAndMore\MyCyclingLog($this->echoCallback);
        $this->endomondo = new JoanMcGalliard\EddingtonAndMore\Endomondo($deviceId, $googleApiKey, $this->preferences->getTimezone(), $this->echoCallback);
        $this->rideWithGps = new JoanMcGalliard\EddingtonAndMore\RideWithGps($googleApiKey, $this->echoCallback);
        ...
        $this->isNotConnectedToAll = !$this->strava->isConnected() || !$this->myCyclingLog->isConnected() || !$this->endomondo->isConnected() || !$this->strava->writeScope();
    ...
    private function execute($state)
    {
        ...
        if ($state == "calculate_from_strava" || $state == "calculate_from_mcl" || $state == "calculate_from_endo" || $state == "calculate_from_rwgps") {
            ...
            if ($state == "calculate_from_strava") {
                ...
                if ($strava->getError()) {
                    $this->output("<br><span style=\"color:red;\">There was a problem getting data from Strava.</span><br><em>"
                        . $strava->getError()
                        . "</em>");
                }
                ...
                if ($error) {
                    $this->output("There was a problem getting data from $source:<br>" . $error);
                }
            ...
            $this->output("<p>According to $source, for the period from $start_text to $end_text, "
            ...
            $eddington_imperial = calculateEddington($days, $result, self::METRE_TO_MILE);
            ...
            $this->output("<br><a href=\"#imperial\">Your imperial Eddington Number</a> is <strong>$eddington_imperial</strong>.<br>");
            if ($end_text == "today") {
                $goals = next_goals($eddington_imperial);
                foreach ($goals as $goal) {
                    $num = number_of_days_to_goal($goal, $days, self::METRE_TO_MILE);
                    $this->output("You need to do $num ride(s) of at least $goal to increase it to $goal.<br>");
                }
            }
            $eddington_metric = calculateEddington($days, $result, self::METRE_TO_KM);
            ...
            $table_metric .= "</table>";
            $this->output("<br><a href=\"#metric\">Your metric Eddington Number</a> is <strong>$eddington_metric</strong><br>");
            if ($end_text == "today") {
                $goals = next_goals($eddington_metric);
                foreach ($goals as $goal) {
                    $num = number_of_days_to_goal($goal, $days, self::METRE_TO_KM);
                    $this->output("You need to do $num ride(s) of at least $goal to increase it to $goal.<br>");
                }
            }

            $this->output('<br><a href="#eddington_chart">See a chart of how your Eddington number has grown over the years.</a><br>');
            $this->output("<p><em>Run time " . (time() - $timestamp) . " seconds.</em></p>");

            $this->output($table_imperial);
            $this->output($table_metric);
            $imperial_history = eddingtonHistory($days, self::METRE_TO_MILE);
            $metric_history = eddingtonHistory($days, self::METRE_TO_KM);

            $this->output(buildChart($imperial_history, $metric_history));
            ...
            if ($strava->getError()) {
                $this->output("<br>There was a problem getting data from Strava.<br>" . $strava->getError());
                $this->output("<br>Please try again");
            } else {
                $count = 0;
                ...
                $this->output("<br>$count rides added.<br>");
                if (sizeof($rides_to_retry) != 0) {
                    $this->output("Some rides failed to be added.  See above.<br>");
                }
            ...
            if (is_int($result)) {
                $this->output("Deleted $result activities from MyCyclingLog");
            } else {
                $this->output("<p style=\"color:red;\"><b>Problem connecting to MyCyclingLog: $result</b></p>");
            }
            ...
            if ($rideWithGps->getError()) {
                $this->output("<br>There was a problem getting data from RideWithGPS.<br>" . $rideWithGps->getError());
                $this->output("<br>Please try again");
            } else {
    ...
    private function displayPage()
    {
        ...
        /** @var \JoanMcGalliard\EddingtonAndMore\Endomondo $endomondo */
        $endomondo=$this->endomondo;
        $this->output("<form action=\"$this->here\" method=\"post\" name=\"main_form\">");
        $this->output('
    <hr>
...
");
        ...
        $this->output("    <table class=\"w3-table-all\">");
        if ($this->isConnected) {
            $this->output("<tr>
        ...
        if ($strava->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="calculate_from_strava" value="Eddington Number from Strava"/><br>');
        ...
        if ($myCyclingLog->isConnected()) {
            ...
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="calculate_from_mcl" value="Eddington Number from MyCyclingLog"/></td></tr>');
        }
        if ($endomondo->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="calculate_from_endo" value="Eddington Number from Endomondo"/><br>');
            echo 'Split multiday rides?:
        ...
        if ($rideWithGps->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="calculate_from_rwgps" value="Eddington Number from RideWithGPS"/><br>');
            echo 'Split multiday rides?:
        ...
        if ($strava->isConnected() && $myCyclingLog->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="copy_strava_to_mcl" value="Copy ride data from Strava to MyCyclingLog"/>  <br>');
        ...
        if ($strava->isConnected() && $endomondo->isConnected() && $strava->writeScope()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="copy_endo_to_strava" value="Copy rides and routes from Endomondo to Strava"/>  <br>');
            $this->output("</td></tr>");
        }
        if ($rideWithGps->isConnected() && $endomondo->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input type="submit" name="copy_endo_to_rwgps" value="Copy rides and routes from Endomondo to RideWithGPS"/>  <br>');
            $this->output("</td></tr>");
        }

        if ($myCyclingLog->isConnected()) {
            $this->output('<tr><td' . $colSpan . '><input onclick="confirm_mcl_deletes()" type="button" name="delete_mcl_rides" value="Delete MyCyclingLog rides"/>');
            $this->output("</td></tr>");
        }
        $this->output(" <tr>
        ...
        if ($this->isNotConnectedToAll) {
            $this->output('<p>More options are available if you connect to <a href="#services">other services</a>.</p>');
        }
        ...
        $this->output(        "function confirm_mcl_deletes() {
        ...
        $this->output("var password = prompt(password_warning);
            if (password != null) {
...
");
            if (!$strava->isConnected() || !$strava->writeScope()) {
                ...
                $this->output("Read/write acccess (only click this if you want to upload rides from Endomondo to Strava): <br>");
                $this->output('<a href="' .
...
");
        $this->output("<p style=\"color:red;\"><b>$this->error_message</b></p>");
        $this->output("<p style=\"color:blueviolet;\"><em>$this->info_message</em></p>");
        $this->output("<p>On this page you can calculate your Eddington Number from your Strava, Endomondo, MyCyclingLog or RideWithGPS
 accounts, or
    transfer rides from Strava
    to MyCyclingLog, from Endomondo to Strava or from Endomondo to RideWithGPS</p>");
        $this->output("<hr>");
        $this->output("<p>The Eddington Number is a metric for long distance cyclists.  It's the largest value of E where you
